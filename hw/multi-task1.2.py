...
def train_epoch(device,batch_size):
    ...
    train_loss5 = []
    #data0_iter.shuffle    
    data0_iter,data1_iter,data2_iter,data3_iter,data4_iter,data5_iter,labels_iter=Handwritten_numerals(True)
...
def test_epoch():
    encoder.eval()
    encoded_samples = []
    data0_iter,data1_iter,data2_iter,data3_iter,data4_iter,data5_iter,labels_iter=Handwritten_numerals(False)
    ...
    y_pred = KMeans(n_clusters=10,n_init=10).fit_predict(X)
    ...
    # 创建GMM对象并拟合数据#展示预测标签    
    gmm = GaussianMixture(n_components=10, max_iter=100)
    gmm.fit(X)
