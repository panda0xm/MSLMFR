...
class Encoder(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.sharelinear1=torch.nn.Linear(size1*6, size2)
        self.sharelinear2=torch.nn.Linear(size2, size3)
        self.linear00=torch.nn.Linear(48, size1)
        self.linear10=torch.nn.Linear(40, size1)
        self.linear20=torch.nn.Linear(254, size1)
        self.linear30=torch.nn.Linear(1984, size1)
...
def train_epoch(device,batch_size):
    ...
    train_loss5 = []
    #data0_iter.shuffle    
    data0_iter,data1_iter,data2_iter,data3_iter,data4_iter,data5_iter,labels_iter=dataset.Cal7(True)
...
def test_epoch(n_clusters):
    encoder.eval()
    encoded_samples = []
    data0_iter,data1_iter,data2_iter,data3_iter,data4_iter,data5_iter,labels_iter=dataset.Cal7(False)
...
def train(num_epochs,batch_size):
    ...
    for epoch in range(1,num_epochs+1):
        [loss0,loss1,loss2,loss3,loss4,loss5] =train_epoch(device, batch_size)
        if(epoch%100==0):
            acc,nmi,purity,f1,precision,recall,ari=test_epoch(7)
            if(acc>max_acc):
                max_acc=acc
                # 保存模型
...
encoder = Encoder()
decoder0 = Decoder(size3,size4,48)
decoder1 = Decoder(size3,size4,40)  
decoder2 = Decoder(size3,size4,254)  
decoder3 = Decoder(size3,size4,1984)  
