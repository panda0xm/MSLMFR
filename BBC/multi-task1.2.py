...
class Encoder(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.sharelinear1=torch.nn.Linear(size1*4, size2)
        self.sharelinear2=torch.nn.Linear(size2, size3)
        self.linear00=torch.nn.Linear(4659, size1)
        self.linear10=torch.nn.Linear(4633, size1)
        self.linear20=torch.nn.Linear(4665, size1)
        self.linear30=torch.nn.Linear(4684, size1)
        #self.linear40=torch.nn.Linear(512, size1)
        #self.linear50=torch.nn.Linear(928, size1)
    ...
    def forward(self, x0,x1,x2,x3):
        ...
        #x4 = (self.linear40(x4))
        #x5 = (self.linear50(x5))
        x_hidden=torch.cat((x0,x1,x2,x3),1)
...
### Training function
def train_epoch(device,batch_size):
    ...
    #train_loss5 = []
    #data0_iter.shuffle    
    data0_iter,data1_iter,data2_iter,data3_iter,labels_iter=dataset.BBC(True)
    ...
    for i in range(int(batch_num)): 
        ...
        #task0
        optimizer.zero_grad()        
        encoded_data = encoder(image_batch0,image_batch1,image_batch2,image_batch3)
    ...
    return [np.mean(train_loss0),np.mean(train_loss1),np.mean(train_loss2),np.mean(train_loss3)]

def test_epoch(n_clusters):
    encoder.eval()
    encoded_samples = []
    data0_iter,data1_iter,data2_iter,data3_iter,labels_iter=dataset.BBC(False)
    for i in range(len(data0_iter)):
        ...
        # Encode image
        with torch.no_grad():
            encoded_img  = encoder(data0,data1,data2,data3)
        # Append to list
...
def train(num_epochs,batch_size):
    diz_loss = {'loss0':[],'loss1':[],'loss2':[],'loss3':[]}
    max_nmi=0.34
    for epoch in range(1,num_epochs+1):
        [loss0,loss1,loss2,loss3] =train_epoch(device, batch_size)
        if(epoch%100==0):
            acc,nmi,purity,f1,precision,recall,ari=test_epoch(5)
            if(nmi>max_nmi):
                max_nmi=nmi
                # 保存模型
                ...
                plt.show()
            print(f'\n EPOCH {epoch}/{num_epochs} \t loss0 {loss0}\t loss1 {loss1}\t loss2 {loss2}\t loss3 {loss3}')
...
encoder = Encoder()
decoder0 = Decoder(size3,size4,4659)
decoder1 = Decoder(size3,size4,4633)  
decoder2 = Decoder(size3,size4,4665)  
decoder3 = Decoder(size3,size4,4684)  
#decoder4 = Decoder(size3,size4,512)  
#decoder5 = Decoder(size3,size4,928)  
