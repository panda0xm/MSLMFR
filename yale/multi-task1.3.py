...
class Encoder(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.sharelinear1=torch.nn.Linear(size1*3, size2)
        self.sharelinear2=torch.nn.Linear(size2, size3)
        self.linear00=torch.nn.Linear(4096, size1)
        self.linear10=torch.nn.Linear(3304, size1)
        self.linear20=torch.nn.Linear(6750, size1)
...
def train_epoch(device,batch_size):
    ...
    #train_loss5 = []
    #data0_iter.shuffle    
    data0_iter,data1_iter,data2_iter,labels_iter=dataset.Yale(True)
    ...
    for i in range(int(batch_num)): 
        ...
        #task0
        optimizer.zero_grad()        
        encoded_data = encoder(image_batch0,image_batch1,image_batch2)
...
def test_epoch(n_clusters):
    encoder.eval()
    encoded_samples = []
    data0_iter,data1_iter,data2_iter,labels_iter=dataset.Yale(False)
    for i in range(len(data0_iter)):
        ...
        # Encode image
        with torch.no_grad():
            encoded_img  = encoder(data0,data1,data2)
        # Append to list
...
def train(num_epochs,batch_size):
    diz_loss = {'loss0':[],'loss1':[],'loss2':[]}
    max_nmi=0.6
    for epoch in range(1,num_epochs+1):
        [loss0,loss1,loss2] =train_epoch(device, batch_size)
        if(epoch%100==0):
            acc,nmi,purity,f1,precision,recall,ari=test_epoch(15)
            if(nmi>max_nmi):
                max_nmi=nmi
                # 保存模型
                ...
                plt.show()
            print(f'\n EPOCH {epoch}/{num_epochs} \t loss0 {loss0}\t loss1 {loss1}\t loss2 {loss2}')
...
encoder = Encoder()
decoder0 = Decoder(size3,size4,4096)
decoder1 = Decoder(size3,size4,3304)  
decoder2 = Decoder(size3,size4,6750)  
