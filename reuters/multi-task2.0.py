...
class Encoder(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.sharelinear1=torch.nn.Linear(size1*5, size2)
        self.sharelinear2=torch.nn.Linear(size2, size3)
        self.linear00=torch.nn.Linear(21531, size1)
        self.linear10=torch.nn.Linear(24892, size1)
        self.linear20=torch.nn.Linear(34251, size1)
        self.linear30=torch.nn.Linear(15506, size1)
        self.linear40=torch.nn.Linear(11547, size1)
    ...
    def forward(self, x0,x1,x2,x3,x4):
        ...
        x3 = (self.linear30(x3))
        x4 = (self.linear40(x4))
        x_hidden=torch.cat((x0,x1,x2,x3,x4),1)
...
def train_epoch(device,batch_size):
    ...
    for i in range(int(batch_num)): 
        ...
        #task0
        optimizer.zero_grad()        
        encoded_data = encoder(image_batch0,image_batch1,image_batch2,image_batch3,image_batch4)
        ...
        alpha=mtl.backward(losses=[loss0,loss1,loss2,loss3,loss4,loss5])
        #print('alpha',alpha)
        optimizer1.step()
        '''
    ...
    np.mean(train_loss3),np.mean(train_loss4)]
...
def test_epoch(n_clusters):
    ...
    for i in range(len(data0_iter)):
        ...
        # Encode image
        with torch.no_grad():
            encoded_img  = encoder(data0,data1,data2,data3,data4)
        # Append to list
...
def train(num_epochs,batch_size):
    diz_loss = {'loss0':[],'loss1':[],'loss2':[],'loss3':[],'loss4':[]}
    max_nmi=0.4
    for epoch in range(1,num_epochs+1):
        printTime()
        [loss0,loss1,loss2,loss3,loss4] =train_epoch(device, batch_size)
        if(epoch%10==0):
            acc,nmi,purity,f1,precision,recall,ari=test_epoch(6)
            if(nmi>max_nmi):
                max_nmi=nmi
                # 保存模型
                ...
                #plt.show()
            print(f'\n EPOCH {epoch}/{num_epochs} \t loss0 {loss0}\t loss1 {loss1}\t loss2 {loss2}\t loss3 {loss3}\t loss4 {loss4}')
...
encoder = Encoder()
decoder0 = Decoder(size3,size4,21531)
decoder1 = Decoder(size3,size4,24892)  
decoder2 = Decoder(size3,size4,34251)  
decoder3 = Decoder(size3,size4,15506)  
decoder4 = Decoder(size3,size4,11547)  
