...
def Yale(shuffle=False):
    ...
    for i in random_list:
        ...
        label_list.append(labels[i][0]-1)
    #print(label_list)
    return data0_list,data1_list,data2_list,label_list
    
def YaleB(shuffle=False):
    # 加载MAT文件
    mat = scipy.io.loadmat('../data/yaleB.mat')
    # 提取cell和double数据
    data = mat['data']
    labels = mat['labels']
    #print(data[0][1])
...
def Reuters(shuffle=False):
    # 加载MAT文件
    mat = scipy.io.loadmat('../data/reuters.mat')
    # 提取cell和double数据
    data = mat['data']
    labels = mat['labels']
    length=len(labels)
    ...
    #data5_list=[]
    label_list=[]
    data0=data[0][0].toarray()
    data1=data[0][1].toarray()
    data2=data[0][2].toarray()
    data3=data[0][3].toarray()
    data4=data[0][4].toarray()
    for i in random_list:
        #print(data[0][0].toarray()[i]);
        #print(image_batch0.shape) 
        #raise Exception()
        data0_list.append(data0[i].astype('float32'))
        data1_list.append(data1[i].astype('float32'))
        data2_list.append(data2[i].astype('float32'))
        data3_list.append(data3[i].astype('float32'))
        data4_list.append(data4[i].astype('float32'))
        #data5_list.append(data[5][0][i].astype('float32'))
        label_list.append(labels[i][0])
    #print(label_list)
    return data0_list,data1_list,data2_list,data3_list,data4_list,label_list